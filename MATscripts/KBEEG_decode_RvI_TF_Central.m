...
%     EEG3 = pop_select(EEG3, 'channel', 1:10); EEG4 = pop_select(EEG4, 'channel', 1:10); 
    ...
    EEG7 = pop_select(EEG7, 'channel', 4:5); 
...
cfg.avgoverfreq = 'yes';
cfg.latency = [-0.3 0.3];
preRI_SG2 = ft_selectdata(cfg,preRI_SG );
postRI_SG2 = ft_selectdata(cfg,postRI_SG );

preRI_WG2 = ft_selectdata(cfg,preRI_WG );
postRI_WG2 = ft_selectdata(cfg,postRI_WG );

figure;plot(preRI_SG2.time, mean(squeeze(preRI_SG2.powspctrm)))
hold on;plot(postRI_SG2.time, mean(squeeze(postRI_SG2.powspctrm)))

figure;plot(preRI_WG2.time, mean(squeeze(preRI_WG2.powspctrm)))
hold on;plot(postRI_WG2.time, mean(squeeze(postRI_WG2.powspctrm)))
